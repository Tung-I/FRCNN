...
class _QuadAttentionRCNN(nn.Module):
    ...
    def __init__(self, classes, attention_type, rpn_reduce_dim, rcnn_reduce_dim, gamma, semantic_enhance, n_way=2, n_shot=5, pos_encoding=True):
        super(_QuadAttentionRCNN, self).__init__()
        ...
        if self.semantic_enhance:
            ...
            self.rcnn_channel_k_layer = nn.Linear(dim_in, 1)
            init.normal_(self.rcnn_channel_k_layer.weight, std=0.01)
            init.constant_(self.rcnn_channel_k_layer.bias, 0)
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, support_ims, all_cls_gt_boxes=None):
        ...
        if self.semantic_enhance:
            query_spatial_weight = self.rpn_channel_k_layer(query_mat)  # [B, h*w, 1]
            query_spatial_weight = F.softmax(query_spatial_weight, 1)
            query_channel_global = torch.bmm(query_spatial_weight.transpose(1, 2), query_mat)  # [B, 1, 1024]
            query_mat = query_mat + self.channel_gamma * query_channel_global
        ...
        for i in range(self.n_shot):
            ...
            if self.semantic_enhance:
                ...
                single_s_mat = single_s_mat + self.channel_gamma * support_channel_global
    ...
    def rcnn_head(self, pooled_feat, support_feat):
        ...
        # query channel enhance
        if self.semantic_enhance:
            query_spatial_weight = self.rcnn_channel_k_layer(query_mat)
            query_spatial_weight = F.softmax(query_spatial_weight, 1)
            query_channel_global = torch.bmm(query_spatial_weight.transpose(1, 2), query_mat)
            query_mat = query_mat + self.channel_gamma * query_channel_global
        ...
        for i in range(self.n_shot):
            ...
            # support channel enhance
            if self.semantic_enhance:
                support_spatial_weight = self.rcnn_channel_k_layer(single_s_mat)
                support_spatial_weight = F.softmax(support_spatial_weight, 1)
                support_channel_global = torch.bmm(support_spatial_weight.transpose(1, 2), single_s_mat)
                single_s_mat = single_s_mat + self.channel_gamma * support_channel_global
...
class QuadAttentionRCNN(_QuadAttentionRCNN):
    def __init__(self, classes, attention_type, rpn_reduce_dim=400, rcnn_reduce_dim=256, gamma=0.1, semantic_enhance=False, 
        ...
        _QuadAttentionRCNN.__init__(self, classes, attention_type, rpn_reduce_dim, rcnn_reduce_dim, gamma, semantic_enhance, 
