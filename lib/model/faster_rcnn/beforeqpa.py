...
class _BEFOREQPARCNN(nn.Module):
    ...
    def __init__(self, classes, attention_type, rpn_reduce_dim, rcnn_reduce_dim, gamma, semantic_enhance, n_way=2, n_shot=5, pos_encoding=True):
        super(_BEFOREQPARCNN, self).__init__()
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, support_ims, all_cls_gt_boxes=None):
        ...
        if self.training:
            ...
            pos_support_feat = support_feats[:, :self.n_shot, :, :, :].mean(1, keepdim=True).contiguous()  # [B, shot, 1024, 20, 20]
            neg_support_feat = support_feats[:, self.n_shot:self.n_way*self.n_shot, :, :, :].mean(1, keepdim=True).contiguous()
            ...
            pos_support_feat_pooled = pos_support_feat_pooled.view(batch_size, 1, 1024, 7, 7)  # [B, shot, 1024, 7, 7]
            neg_support_feat_pooled = neg_support_feat_pooled.view(batch_size, 1, 1024, 7, 7)
        else:
            support_ims = support_ims.view(-1, support_ims.size(2),  support_ims.size(3),  support_ims.size(4))
            ...
            pos_support_feat = support_feats[:, :self.n_shot, :, :, :].mean(1, keepdim=True)
            pos_support_feat_pooled = self.avgpool(pos_support_feat.view(-1, 1024, 20, 20))
            pos_support_feat_pooled = pos_support_feat_pooled.view(batch_size, 1, 1024, 7, 7)

        batch_size = pos_support_feat.size(0)
        feat_h = base_feat.size(2)
        feat_w = base_feat.size(3)
        support_mat = pos_support_feat.transpose(0, 1).view(1, batch_size, 1024, -1).transpose(2, 3)  # [shot, B, 400, 1024]
    ...
    def rcnn_head(self, pooled_feat, support_feat):
        ...
        for query_feat, target_feat in zip(pooled_feat.chunk(batch_size, dim=0), support_feat.chunk(batch_size, dim=0)):
            ...
            target_feat = target_feat.view(1, 1, 1024, -1).transpose(2, 3)  # [1, shot, 49, c]
            ...
            if self.pos_encoding:
                target_feat = self.pos_encoding_layer(target_feat.view(-1, 49, 1024)).view(-1, 1, 49, 1024)
...
class BEFOREQPARCNN(_BEFOREQPARCNN):
    def __init__(self, classes, attention_type, rpn_reduce_dim=400, rcnn_reduce_dim=256, gamma=0.1, semantic_enhance=False, 
        ...
        _BEFOREQPARCNN.__init__(self, classes, attention_type, rpn_reduce_dim, rcnn_reduce_dim, gamma, semantic_enhance, 
