...
class _divergentAttention(nn.Module):
    """ faster RCNN """
    def __init__(self, classes, n_head, attention_type, shot_mode, loss_func, n_way=2, n_shot=5, pos_encoding=True, gamma=1.0):
        super(_divergentAttention, self).__init__()
        ...
        if self.attention_type == 'concat':
            if self.loss_func == 'prime':
                self.RCNN_rpn = _PrimeRPN(2048, gamma)
            elif self.loss_func == 'normal':
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, support_ims, all_cls_gt_boxes=None):
        ...
        for n in range(self.n_head):
            ...
            for i in range(self.n_shot):
                ...
                k_mat_support = self.rpn_K_layers[n](s_mat)
    ...
    def rcnn_head(self, pooled_feat, support_feat):
        ...
        for n in range(self.n_head):
            ...
            for i in range(self.n_shot):
                ...
                _k = self.rcnn_K_layers[n](support_mat[i])
...
class DA(_divergentAttention):
    def __init__(self, classes, n_head, attention_type, shot_mode, loss_func, num_layers=50, pretrained=False, num_way=2, num_shot=5, pos_encoding=True):
        self.model_path = 'data/pretrained_model/resnet50_caffe.pth'
        self.dout_base_model = 1024
        self.pretrained = pretrained
        _divergentAttention.__init__(self, classes, n_head, attention_type, shot_mode, loss_func, n_way=num_way, n_shot=num_shot, pos_encoding=pos_encoding)
