...
class _multishotRCNN(nn.Module):
    """ faster RCNN """
    def __init__(self, classes, n_head, attention_type, mode, n_way=2, n_shot=5, pos_encoding=True, d_k=64):
        super(_multishotRCNN, self).__init__()
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, support_ims, all_cls_gt_boxes=None):
        ...
        attentive_support_feat = torch.cat(attentive_support_feat, 2)  # [B, hw, n_head*shot*1024]
        if self.n_head != 1:
...
class MultishotRCNN(_multishotRCNN):
    def __init__(self, classes, n_head, attention_type, mode, num_layers=50, pretrained=False, num_way=2, num_shot=5, pos_encoding=True, d_k=64):
        self.model_path = 'data/pretrained_model/resnet50_caffe.pth'
        self.dout_base_model = 1024
        self.pretrained = pretrained
        _multishotRCNN.__init__(self, classes, n_head, attention_type, mode, n_way=num_way, n_shot=num_shot, pos_encoding=pos_encoding, d_k=d_k)
