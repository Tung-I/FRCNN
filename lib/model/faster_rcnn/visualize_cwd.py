...
class _CWDPAARCNN(nn.Module):
    ...
    def __init__(self, classes, attention_type, rpn_reduce_dim, rcnn_reduce_dim, gamma, semantic_enhance, n_way=2, n_shot=5, pos_encoding=True):
        super(_CWDPAARCNN, self).__init__()
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, support_ims, all_cls_gt_boxes=None):
        ...
        for i in range(self.n_shot):
            ...
            single_s_mat_ori = single_s_mat.clone()
            if self.semantic_enhance:
                ...
                single_s_mat = single_s_mat + 0.5 * F.leaky_relu(support_channel_global)

            ############################################################################################################
            np.save('/home/tony/Desktop/attention/cwd.npy', support_spatial_weight.view(20, 20).detach().cpu().numpy())
            np.save('/home/tony/Desktop/attention/smat.npy', single_s_mat_ori.view(20, 20, 1024).detach().cpu().numpy())
            np.save('/home/tony/Desktop/attention/enhance_smat.npy', single_s_mat.view(20, 20, 1024).detach().cpu().numpy())
            np.save('/home/tony/Desktop/attention/channel.npy', support_channel_global.squeeze().detach().cpu().numpy())
            print('SE saved')
            ############################################################################################################
...
class VisualizeModel_CWD(_CWDPAARCNN):
    def __init__(self, classes, attention_type, rpn_reduce_dim=400, rcnn_reduce_dim=256, gamma=0.1, semantic_enhance=False, 
        ...
        _CWDPAARCNN.__init__(self, classes, attention_type, rpn_reduce_dim, rcnn_reduce_dim, gamma, semantic_enhance, 
