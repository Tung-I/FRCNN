...
class _originRCNN(nn.Module):
    """ faster RCNN """
    def __init__(self, classes, n_head, attention_type, n_way=2, n_shot=5, pos_encoding=True, d_k=64):
        super(_originRCNN, self).__init__()
        ...
        self.rpn_Q_layers = nn.ModuleList(Q_list)
        self.rpn_K_layers = nn.ModuleList(K_list)
        Q_list = []
        K_list = []
        for i in range(n_head):
            Q_weight = nn.Linear(dim_in, self.rcnn_d)
            K_weight = nn.Linear(dim_in, self.rcnn_d)
            init.normal_(Q_weight.weight, std=0.01)
            init.constant_(Q_weight.bias, 0)
            init.normal_(K_weight.weight, std=0.01)
            init.constant_(K_weight.bias, 0)
            Q_list.append(Q_weight)
            K_list.append(K_weight)
        self.rcnn_Q_layers = nn.ModuleList(Q_list)
        self.rcnn_K_layers = nn.ModuleList(K_list)
        # rpn
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, support_ims, all_cls_gt_boxes=None):
        ...
        if self.training:
            ...
            neg_support_feat = support_feats[:, self.n_shot:self.n_way*self.n_shot, :, :, :].mean(1)  # [B, 1024, 20, 20]
            pos_support_feat_pooled = self.avgpool(pos_support_feat)  # [B, 1024, 7, 7]
            neg_support_feat_pooled = self.avgpool(neg_support_feat)
        else:
            support_ims = support_ims.view(-1, support_ims.size(2),  support_ims.size(3),  support_ims.size(4))
            support_feats = self.RCNN_base(support_ims)
            support_feats = support_feats.view(-1, self.n_shot, support_feats.size(1), support_feats.size(2), support_feats.size(3))
            pos_support_feat = support_feats[:, :self.n_shot, :, :, :].mean(1)
            pos_support_feat_pooled = self.avgpool(pos_support_feat)

        # attention RPN
        batch_size = pos_support_feat.size(0)
        feat_h = base_feat.size(2)
        feat_w = base_feat.size(3)
        support_mat = pos_support_feat.view(batch_size, self.pool_feat_dim, -1).transpose(1, 2)  # [B, 400, 1024]
        ...
        for n in range(self.n_head):
            q_mat_query = self.rpn_Q_layers[n](query_mat)
            k_mat_support = self.rpn_K_layers[n](support_mat)
            qk_mat = torch.bmm(q_mat_query, k_mat_support.transpose(1, 2)) / math.sqrt(self.rpn_d)  # [B, hw, 400]
            qk_mat = F.softmax(qk_mat, dim=2)
            z_mat_support = torch.bmm(qk_mat, support_mat)  # [B, hw, 1024]
            attentive_support_feat += [z_mat_support]
        
        ############################################################################################################
        heat_mat_to_save = qk_mat.view(1, feat_h, feat_w, 20, 20).squeeze().detach().cpu().numpy()
        np.save('/home/tony/Desktop/attention/heat_map.npy', heat_mat_to_save)
        np.save('/home/tony/Desktop/attention/im_data.npy', im_data.squeeze().permute(1, 2, 0).detach().cpu().numpy())
        np.save('/home/tony/Desktop/attention/sup_data.npy', support_ims.squeeze().permute(1, 2, 0).detach().cpu().numpy())
        raise Exception(' ')
        ############################################################################################################
    ...
    def rcnn_head(self, pooled_feat, support_feat):
        ...
        multihead_feat = []
        for n in range(self.n_head):
            _q = self.rcnn_Q_layers[n](query_mat)  # [B*128, 49, rcnn_d]
            _k = self.rcnn_K_layers[n](support_mat)
            _qk = torch.bmm(_q, _k.transpose(1, 2)) / math.sqrt(self.rcnn_d)  # [B*128, 49, 49]
            _qk = F.softmax(_qk, dim=2)
            _z = torch.bmm(_qk, support_mat)  # [B*128, 49, c]
            multihead_feat += [_z]
...
class OriginRCNN(_originRCNN):
    def __init__(self, classes, n_head, attention_type, num_layers=50, pretrained=False, num_way=2, num_shot=5, pos_encoding=True, d_k=64):
        self.model_path = 'data/pretrained_model/resnet50_caffe.pth'
        self.dout_base_model = 1024
        self.pretrained = pretrained
        _originRCNN.__init__(self, classes, n_head, attention_type, n_way=num_way, n_shot=num_shot, pos_encoding=pos_encoding, d_k=d_k)
