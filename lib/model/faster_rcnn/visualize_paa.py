...
class _TripleARCNN(nn.Module):
    """ Auto-Adaptive-Attention """
    def __init__(self, classes, n_head, attention_type, shot_mode, n_way=2, n_shot=5, pos_encoding=True, gamma=0.1):
        super(_TripleARCNN, self).__init__()
        ...
        self.adapt_M_layers = nn.ModuleList(M_list)
        # rpn
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, support_ims, all_cls_gt_boxes=None):
        ...
        for n in range(self.n_head):
            ...
            for i in range(self.n_shot):
                ...
                q_mat_query = q_mat_query - q_mat_query.mean(1, keepdim=True)
                k_mat_support = k_mat_support - k_mat_support.mean(1, keepdim=True)
                qk_mat = torch.bmm(q_mat_query, k_mat_support.transpose(1, 2)) / math.sqrt(self.rpn_d)  # [B, hw, 400]
                qk_mat = F.softmax(qk_mat, dim=2)
                # unary term
                unary_term = self.adapt_M_layers[n](s_mat)  # [B, 400, 1]
                ...
                np.save('/home/tony/Desktop/attention/unary.npy', unary_term.squeeze().view(20, 20).detach().cpu().numpy())
                print('paa saved')
                ############################################################################################################
                # disentangled attention map
                qk_mat = qk_mat + unary_term.transpose(1, 2) * self.gamma
    ...
    def rcnn_head(self, pooled_feat, support_feat):
        ...
        for n in range(self.n_head):
            ...
            for i in range(self.n_shot):
                ...
                _q = _q - _q.mean(1, keepdim=True)
                _k = _k - _k.mean(1, keepdim=True)
                _qk = torch.bmm(_q, _k.transpose(1, 2)) / math.sqrt(self.rcnn_d)  # [B*128, 49, 49]
                _qk = F.softmax(_qk, dim=2)
                # unary term
                unary_term = self.adapt_M_layers[n](support_mat[i])
                unary_term = F.softmax(unary_term, dim=1)
                # disentangled attention map
                _qk = _qk + unary_term.transpose(1, 2) * self.gamma
...
class VisualizeModel_PAA(_TripleARCNN):
    def __init__(self, classes, n_head, attention_type, shot_mode, num_layers=50, pretrained=False, num_way=2, num_shot=5, pos_encoding=True):
        ...
        _TripleARCNN.__init__(self, classes, n_head, attention_type, shot_mode, n_way=num_way, n_shot=num_shot, pos_encoding=pos_encoding)
