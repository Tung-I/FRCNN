...
class _OnewayAdaptiveAttention(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, support_ims, all_cls_gt_boxes=None):
        ...
        pos_support_feat = support_feats[:, :self.n_shot, :, :, :].contiguous()
        ...
        if self.training:
            ...
            RCNN_loss_cls = F.cross_entropy(cls_score_all, rois_label)
        else:
            RCNN_loss_cls = 0
            RCNN_loss_bbox = 0
...
class OnewayAAARCNN(_OnewayAdaptiveAttention):
    def __init__(self, classes, n_head, attention_type, shot_mode, num_layers=50, pretrained=False, num_way=2, num_shot=5, pos_encoding=True):
        ...
        _OnewayAdaptiveAttention.__init__(self, classes, n_head, attention_type, shot_mode, n_way=num_way, n_shot=num_shot, pos_encoding=pos_encoding)
