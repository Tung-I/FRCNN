...
from model.rpn.prime_rpn import _PrimeRPN
...
class _primeRCNN(nn.Module):
    """ faster RCNN """
    def __init__(self, classes, n_head, attention_type, gamma, n_way=2, n_shot=5, pos_encoding=True, d_k=64):
        super(_primeRCNN, self).__init__()
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, support_ims, all_cls_gt_boxes=None):
        ...
        rois, rpn_loss_cls, rpn_loss_bbox = self.RCNN_rpn(correlation_feat, im_info, gt_boxes, num_boxes, all_cls_gt_boxes)
...
class PrimeRCNN(_primeRCNN):
    def __init__(self, classes, n_head, attention_type, loss_gamma, num_layers=50, pretrained=False, num_way=2, num_shot=5, pos_encoding=True, d_k=64):
        self.model_path = 'data/pretrained_model/resnet50_caffe.pth'
        self.dout_base_model = 1024
        self.pretrained = pretrained
        _primeRCNN.__init__(self, classes, n_head, attention_type, loss_gamma, n_way=num_way, n_shot=num_shot, pos_encoding=pos_encoding, d_k=d_k)
